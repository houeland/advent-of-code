...
function dorule(text, pfrom, pto) {
  return gensub(pfrom, pto, "g", text)
}
...
function compute_answer(text) {
  n=split(text, chars, "")
  for (i=1; i<=n; i++) {
    counts[chars[i]]++
  }
  highest_c = -1000000000
  lowest_c = 1000000000
  for (c in counts) {
    print c, counts[c]
...
BEGIN {
  ...
  for (step = 1; step <= 10; step++) {
    ...
    do {
      expanded = updated
      updated = substrules_example(expanded)
    } while (updated != expanded)
    # print updated
    word = updated
    # print "After step " step ":" word " (" length(word) ")"
    print "After step " step ": ... (" length(word) ")"
  }
  compute_answer(word)
  ...
  for (step = 1; step <= 10; step++) {
    ...
    do {
      expanded = updated
      updated = substrules_problem(expanded)
    } while (updated != expanded)
    # print updated
    word = updated
    # print "After step " step ":" word " (" length(word) ")"
